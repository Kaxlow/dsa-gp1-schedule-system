...
class DynamicArray:
  ...
  # First, define decorator to remove Nones from array before sorting and then add the Nones back after sorting
  def drop_add_nones(sort_func):
    def wrapper(self, *args, **kwargs):
      # Operate on arr, a copy of self._collection
      arr = self._collection
      # Count number of Nones
      none_count = 0
      # Drop Nones from the end of the array
      while arr[-1] is None:
        arr = arr[:(len(arr) - 1)]
        none_count += 1
      # Run the sort function on the array
      sorted_arr = sort_func(self, arr, *args, **kwargs)
  ...
  @drop_add_nones
  def insertion_sort(self, arr = [], sort_by = "date_time"):
    # When the function is first called, arr is assigned [] as a placeholder. After calling the @drop_add_nones decorator, arr will be a copy of self._collection with Nones dropped, which the sort will operate on.
    # Ensure that user does not overwrite the initial arr argument
    if arr != []:
      return "Error: Do not provide arr argument."
    ...
    for outer in range(1, len(arr)):
      ...
      arr[inner] = temp
      # Return sorted array to @drop_add_nones decorator to append Nones back
    return arr
  
  # Merge sort. By default, sort by event date and time
  @drop_add_nones
  def merge_sort(self, arr = [], sort_by = "date_time"):
    # When the function is first called, arr is assigned [] as a placeholder. After calling the @drop_add_nones decorator, arr will be a copy of self._collection with Nones dropped, which the sort will operate on.
    # Ensure that user does not overwrite the initial arr argument
    if arr != []:
      return "Error: Do not provide arr argument."
    # If array has multiple elements, split into two recursively
    # If array is left with one element, it is unchanged and returns as sorted_arr
    if len(arr) > 1:
      left_arr = arr[:len(arr // 2)]
      right_arr = arr[(len(arr) // 2):]
      self.merge_sort(left_arr, sort_by)
      self.merge_sort(right_arr, sort_by)
      # Merge split arr in sort order
      ...
      while i < len(left_arr) and j < len(right_arr):
        # If first element of left_arr's sort-by attribute <= first element of right_arr's sort-by attribute, move first element of left_arr into sorted_arr
        if self._set_sort_attr(left_arr[i], sort_by) <= self._set_sort_attr(right_arr[j], sort_by):
          sorted_arr[k] = left_arr[i]
          i += 1
          k += 1
        # Else move the first element of right_arr into sorted_arr
        else:
  ...
  @drop_add_nones  
  def quick_sort(self, arr = [], sort_by = "date_time"):
    # When the function is first called, arr is assigned [] as a placeholder. After calling the @drop_add_nones decorator, arr will be a copy of self._collection with Nones dropped, which the sort will operate on.
    # Ensure that user does not overwrite the initial arr argument
    if arr != []:
      return "Error: Do not provide arr argument."
    # Base case: return array unchanged if there's zero or one element
      ...
      sorted_arr = self.quick_sort(smaller, sort_by) + center + self.quick_sort(larger, sort_by)
      return sorted_arr
  ...
  def binary_search(self, id):
    ...
    self._collection = self.merge_sort(sort_by = "id")
