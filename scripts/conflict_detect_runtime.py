...
# Function to measure conflict detection runtime in different scenarios and get results in array
def get_conflict_detect_runtime():
  # Store results as an array with 3 columns for data_structure, num_events, runtime
  conflict_detect_results = []
  ...
  for n in (50, 500, 2500):
    ...
    # Conflict detection on dynamic array
    runtime_start = time.time()
    da.conflict_detect(new_event)
    runtime = time.time() - runtime_start
    conflict_detect_results.append(["dynamic array", n, runtime])

    # Conflict detection on linked list
    runtime_start = time.time()
    ll.conflict_detect(new_event)
    runtime = time.time() - runtime_start
    conflict_detect_results.append(["linked list", n, runtime])

  return conflict_detect_results

get_conflict_detect_runtime()